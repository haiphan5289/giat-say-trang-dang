...
const WARD = "Gò Vấp";
const OLD_WARDS = "Phường 10 và Phường 17 cũ";
const SLUG = "giat-say-phuong-go-vap";

export const metadata: Metadata = {
  title: `Giặt Sấy Phường ${WARD} (Mới) - Nhận Giao Tận Nơi`,
  description: `Dịch vụ giặt sấy tại phường ${WARD} mới (sáp nhập từ ${OLD_WARDS}). Nhận giao tận nơi trong ${BUSINESS.responseTime}. Từ ${BUSINESS.priceFrom}. Hotline: ${BUSINESS.hotline}.`,
  keywords: [
  ...
  openGraph: {
    title: `Giặt Sấy Phường ${WARD} (Mới) - Nhận Giao Tận Nơi`,
    description: `Giặt sấy chuyên nghiệp phục vụ phường ${WARD} mới. Từ ${BUSINESS.priceFrom}. Nhận giao tận nơi.`,
    url: `https://www.giatsay24hgovap.com/${SLUG}`,
    siteName: "Giặt Sấy 24h Gò Vấp",
    images: [{ url: "/images/shop-front-1.jpg", width: 815, height: 1200, alt: `Giặt Sấy Phường ${WARD} Mới` }],
  },
  twitter: {
    card: "summary_large_image",
    title: `Giặt Sấy Phường ${WARD} (Mới) - Nhận Giao Tận Nơi`,
    description: `Giặt sấy chuyên nghiệp phục vụ phường ${WARD} mới. Từ ${BUSINESS.priceFrom}.`,
  },
};

const faqs: FAQItem[] = [
  {
    q: `Phường ${WARD} mới có phải là toàn bộ quận Gò Vấp không?`,
    a: `Không. Phường ${WARD} là 1 phường mới trong khu vực, được sáp nhập từ ${OLD_WARDS}. Trùng tên với tên quận cũ nên dễ nhầm lẫn, nhưng đây chỉ là 1 phường trong nhiều phường của khu vực Gò Vấp.`,
  },
  {
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 2, name: `Giặt Sấy Phường ${WARD} (Mới)`, item: `https://www.giatsay24hgovap.com/${SLUG}` },
  ],
};
    ...
    <main className="min-h-screen bg-white">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10">
          <FadeIn direction="up">
            ...
            <div className="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-2 text-sm text-cyan-200 font-medium mb-6">
              <MapPin size={14} /> Phục vụ Phường {WARD} (Mới)
            </div>
          </FadeIn>
          <FadeIn direction="up" delay={0.15}>
            <h1 className="text-4xl lg:text-5xl font-extrabold text-white leading-tight mb-6">
              Giặt Sấy Phường {WARD} (Mới) —{" "}
              <span className="shimmer-text">Nhận Giao Tận Nơi</span>
            </h1>
            <p className="text-slate-300 text-lg leading-relaxed mb-8 max-w-2xl">
              Dịch vụ giặt sấy chuyên nghiệp phục vụ Phường {WARD} mới ({OLD_WARDS}). Nhận và giao tận nhà, từ <strong className="text-white">{BUSINESS.priceFrom}</strong>.
            </p>
            <ul className="space-y-2 mb-10">
              {[`Phục vụ toàn bộ khu vực Phường ${WARD} mới (${OLD_WARDS})`, `Lấy đồ trong ${BUSINESS.responseTime}, giao đúng hẹn`, "Miễn phí nhận & giao tận nơi"].map((p) => (
      ...
      <section className="py-16 bg-white">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <CTABanner title={`Đặt Lịch Giặt Sấy Tại Phường ${WARD} (Mới)`} description={<>Nhận giao tận nơi, không cần ra ngoài. Hotline <strong>{BUSINESS.hotline}</strong>.</>} className="mt-10" />
        </div>
      </section>
